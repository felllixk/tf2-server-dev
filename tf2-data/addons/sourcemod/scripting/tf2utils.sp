...
public void OnPluginStart()
{
    ...
    {
        SetFailState("Failed to set up call to " ... "CBaseCombatCharacter::Weapon_ShootPosition()");
    }
    ...
    if (!pNumConds)
    {
        LogError("Could not determine location to read TF_COND_LAST from.  " ... "Condition bounds checking will produce false positives and condition " ... "count native will report incorrect values.");
        g_nConditions = 0xFF;
    }
        ...
        LogError("TF_COND_LAST is not within expected bounds (found %08x).  " ... "Condition bounds checking will produce false positives and condition " ... "count native will report incorrect values.", g_nConditions);
        g_nConditions = 0xFF;
    }
    ...
    if (view_as<int>(offs_CTFPlayer_pCurrentCommand) & ~0xFFFF)
    {
        ...
        SetFailState("Could not determine offset of CTFPlayer::m_pCurrentCommand " ... "(received %08x)", offs_CTFPlayer_pCurrentCommand);
    }
    ...
    {
        SetFailState("Could not determine location to read CTFPlayer::m_flRespawnTimeOverride " ... "from.");
    }
    ...
    {
        // high bits are set - bad read?
        SetFailState("Could not determine offset of CTFPlayer::m_flRespawnTimeOverride " ... " (received %08x)", offs_CTFPlayer_flRespawnTimeOverride);
    }
    ...
    if (!GameConfGetKeyValue(hGameConf, "DOTDamageTypes", damageTypes, sizeof(damageTypes)))
    {
        ...
        for (int c, i, res; (i = StringToIntEx(damageTypes[c], res, 0x10)); c += i)
        {
            /**
            ...
            {
                SetFailState("Not enough space allocated to parse damage types (limit %d) - " ... "update MAX_DOT_DAMAGE_TYPES and recompile", MAX_DOT_DAMAGE_TYPES);
            }
            else if (res == 0) {
                continue;
            }
...
int Native_SetPlayerActiveWeapon(Handle plugin, int nParams)
{
    ...
    return SDKCall(g_SDKCallPlayerWeaponSwitch, client,
                   weapon, 0 /* viewmodelindex; unused */)
        != 0;
}
...
void Native_EquipPlayerWearable(Handle plugin, int numParams)
{
    ...
    if (GetEntPropEnt(wearable, Prop_Send, "m_hOwnerEntity") != client)
    {
        ...
        ThrowNativeError(SP_ERROR_NATIVE,
                         "Assertion failed - wearable entity %d not attached to player. " ... "Gamedata may need to be updated", wearable);
    }
}
